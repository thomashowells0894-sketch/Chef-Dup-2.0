 ...
 *
 * @param {Object} dayData - The dayData object from MealContext keyed by date string.
 *                           Each value has { meals: { breakfast:[], lunch:[], dinner:[], snacks:[] }, totals }
 * @param {{ start: Date, end: Date }} dateRange - Start and end dates for the export range.
 * @returns {Promise<boolean>} true on success, false on failure.
 */
export async function exportFoodDiaryCSV(dayData, dateRange) {
  try {
    ...
    const csvHeader = 'Date,Meal,Food Name,Calories,Protein (g),Carbs (g),Fat (g),Serving\n';
    let csvRows = '';
      ...
      if (day && day.meals) {
        ...
        for (const mealType of mealTypes) {
          ...
          for (const item of items) {
            ...
            const foodName = escapeCSV(item.name || 'Unknown');
            const serving = escapeCSV(item.serving || item.servingSize ? `${item.servingSize || ''} ${item.servingUnit || ''}`.trim() : '1 serving');
    ...
    const fileName = `VibeFit_FoodDiary_${startLabel}_to_${endLabel}.csv`;
 ...
 *
 * @param {Object} weekData - Object containing:
 *   - dailyData: Array of { date, day, calories, protein, carbs, fat, goal }
 *   - stats: { avgCalories, avgProtein, daysTracked, totalCalories, totalProtein }
 *   - streak: number
 *   - goals: { calories, protein, carbs, fat }
 * @param {Object} profile - The user profile from ProfileContext.
 * @returns {Promise<boolean>} true on success, false on failure.
 */
export async function exportWeeklySummaryPDF(weekData, profile) {
  try {
    ...
    const dates = dailyData.map((d) => d.date).filter(Boolean);
    ...
    // Weight trend info
    const weightInfo = profile?.weight
      ? `<div class="stat-card">
    ...
    const dailyRows = dailyData.map((d) => {
      const cals = d.calories || 0;
      const goalVal = d.goal || goals?.calories || 2000;
      const diff = cals - goalVal;
      const diffClass = diff <= 0 ? 'positive' : 'negative';
      const diffLabel = diff <= 0 ? `${Math.abs(diff)} under` : `${diff} over`;
      return `
        <tr>
...
<body>
  ...
  <div class="footer">
    ...
    const pdfFileName = `VibeFit_WeeklyReport_${startDate}_to_${endDate}.pdf`;
