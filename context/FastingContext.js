 ...
 * @param {number|null} startTime - Timestamp when fast started
 * @param {number} durationHours - Target duration in hours
 * @returns {object} - { elapsed, remaining, progress, isComplete }
 */
export function calculateFastingProgress(startTime, durationHours) {
  if (!startTime) {
    return {
      elapsedMs: 0,
...
export function FastingProvider({ children }) {
  ...
  const [fastDuration, setFastDuration] = useState(DEFAULT_FAST_DURATION);
  ...
  // Load saved fasting state on mount
  useEffect(() => {
  ...
  // Auto-save fasting state when it changes (debounced by 500ms)
  useEffect(() => {
    if (!isHydrated) return;

    const timeoutId = setTimeout(() => {
      async function saveFastingState() {
        try {
  ...
  // Start a new fast
  const startFast = useCallback(async (durationHours = null) => {
  ...
  const recordMealLogged = useCallback((mealType) => {
    const now = Date.now();
    setLastMealTime(now);
  ...
  const fastingState = useMemo(() => {
    return isFasting ? FASTING_STATES.FASTING : FASTING_STATES.EATING;
  }, [isFasting]);
